...
class TrackingCamera(CustomCamera):
    ...
    def check_positions(self, cam_result, walker_pos):
        walker_result = self.shoot_a_ray(
            walker_pos, walker_pos + Vec3(0, 0, -3), Model.SENSOR.mask)

        if (not cam_result and walker_result) or (cam_result and not walker_result):
            return True
